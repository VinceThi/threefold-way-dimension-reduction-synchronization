...
# Time parameters
t0, t1, dt = 0, 100, 0.05   # stiff at p_in = 0, even if dt = 0.001
...
I_ext = np.concatenate([I1*np.ones(n1), I2*np.ones(n2)])
...
r1_matrix, r2_matrix, R1_matrix, R2_matrix, r_matrix,  R_matrix = \
    get_synchro_transition_theta_2D(p_out_array, p_in, sizes, nb_instances,
...
# with open('data/theta/2019_06_19_00h13min39sec_test2'
...
if messagebox.askyesno("Python",
    ...
    fig.savefig("data/theta/"
                "{}_{}_complete_vs_reduced_theta_2D"
                ".png".format(timestr, file))

    f = open('data/theta/{}_{}_theta_2D.txt'.format(timestr, file), 'w')
    f.writelines(
    ...
    with open('data/theta/{}_{}_complete_r_matrix_theta_2D'   
              '.json'.format(timestr, file), 'w') as outfile:
        json.dump(r_matrix.tolist(), outfile)
    with open('data/theta/{}_{}_complete_r1_matrix_theta_2D'  
              '.json'.format(timestr, file), 'w') as outfile:
        json.dump(r1_matrix.tolist(), outfile)
    with open('data/theta/{}_{}_complete_r2_matrix_theta_2D'  
              '.json'.format(timestr, file), 'w') as outfile:
        json.dump(r2_matrix.tolist(), outfile)

    with open('data/theta/{}_{}_reduced_R_matrix_theta_2D'    
              '.json'.format(timestr, file), 'w') as outfile:
        json.dump(R_matrix.tolist(), outfile)
    with open('data/theta/{}_{}_reduced_R1_matrix_theta_2D'   
              '.json'.format(timestr, file), 'w') as outfile:
        json.dump(R1_matrix.tolist(), outfile)
    with open('data/theta/{}_{}_reduced_R2_matrix_theta_2D'   
              '.json'.format(timestr, file), 'w') as outfile:
        json.dump(R2_matrix.tolist(), outfile)
